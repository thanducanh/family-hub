...
// 1. Fix types
code = code.replace(/theme\?: "light" \| "dark" \| "system"/g, 'theme?: Theme');
code = code.replace(/setTheme\?: React.Dispatch<React.SetStateAction<"light" \| "dark" \| "system">>/g, 'setTheme?: React.Dispatch<React.SetStateAction<Theme>>');

// 2. Fix guest nav
let loginScreenStart = code.indexOf('function LoginScreen(');
let guestHomeNavStart = code.indexOf('<nav className="fixed bottom-0', loginScreenStart);
let guestHomeNavEnd = code.indexOf('</nav>', guestHomeNavStart) + 6;
let guestNavCode = code.substring(guestHomeNavStart, guestHomeNavEnd);
guestNavCode = guestNavCode.replace(/bg-\[#012f2d\]\/92/g, 'bg-[#800020]');
guestNavCode = guestNavCode.replace(/active \? "text-\[#facc15\]" : "text-\[#cbd5e1\]"/g, 'active ? "text-[#D4AF37]" : "text-[#FFFFFF]"');
code = code.substring(0, guestHomeNavStart) + guestNavCode + code.substring(guestHomeNavEnd);

// 3. Fix MobileNav
let mobileNavStart = code.indexOf('function MobileNav(');
let mobileNavEnd = code.indexOf('</nav>', mobileNavStart) + 6;
let mobileNavCode = code.substring(mobileNavStart, mobileNavEnd);
mobileNavCode = mobileNavCode.replace(/bg-\[#012f2d\]\/92/g, 'bg-[#800020]');
mobileNavCode = mobileNavCode.replace(/const activeColor = "text-\[#facc15\]";/g, 'const activeColor = "text-[#D4AF37]";');
mobileNavCode = mobileNavCode.replace(/const inactiveColor = "text-\[#cbd5e1\]";/g, 'const inactiveColor = "text-[#FFFFFF]";');
code = code.substring(0, mobileNavStart) + mobileNavCode + code.substring(mobileNavEnd);

// 4. Fix MobileHome
let mobileHomeStart = code.indexOf('function MobileHome(');
let desktopHomeStart = code.indexOf('function DesktopHome(', mobileHomeStart);
let mobileHomeCode = code.substring(mobileHomeStart, desktopHomeStart);
...
code = code.substring(0, mobileHomeStart) + mobileHomeCode + code.substring(desktopHomeStart);
