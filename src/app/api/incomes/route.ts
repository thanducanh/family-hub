  ...
  try {
    ...
    for (const row of rows) {
      const result = await client.query(`INSERT INTO income_records
        ...
        RETURNING *`, [row.id, actor.memberId || null, row.incomeDate, row.year, row.month, row.category, row.name, row.amount, row.status, row.note]);
...
export async function PUT(request: NextRequest) {
  ...
  const result = await pool.query(`UPDATE income_records SET
    ...
    WHERE id=$1 RETURNING *`, [row.id, actor.memberId || null, row.incomeDate, row.year, row.month, row.category, row.name, row.amount, row.status, row.note]);
