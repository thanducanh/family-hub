...
export async function GET(request: Request, context: { params: Promise<{ txId: string }> }) {
  ...
  if (!txId) return NextResponse.json({ ok: false, error: "Missing transaction ID" }, { status: 400 });

  const result = await pool.query(
    `SELECT * FROM card_pending_transactions WHERE payment_transaction_id = $1 ORDER BY date DESC, created_at DESC`,
    [txId]
  );
