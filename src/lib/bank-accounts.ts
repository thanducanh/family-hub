...
export async function ensureBankAccountsTable() {
  ...
  `);
  const alter = [
    "ALTER TABLE bank_accounts ADD COLUMN IF NOT EXISTS account_type TEXT DEFAULT 'credit_card'",
    "ALTER TABLE bank_accounts ADD COLUMN IF NOT EXISTS card_network TEXT DEFAULT 'none'",
  ...
  await pool.query(`UPDATE bank_accounts SET last4 = RIGHT(REGEXP_REPLACE(card_number, '\\D', '', 'g'), 4) WHERE (last4 IS NULL OR last4 = '') AND card_number IS NOT NULL AND card_number != ''`);
  await pool.query(`UPDATE bank_accounts SET last4 = RIGHT(REGEXP_REPLACE(account_number, '\\D', '', 'g'), 4) WHERE (last4 IS NULL OR last4 = '') AND account_number IS NOT NULL AND account_number != ''`);
  await pool.query(`UPDATE bank_accounts SET display_name = COALESCE(NULLIF(product_name, ''), bank_name) WHERE display_name IS NULL OR display_name = ''`);
