...
function JobDetail({ job, records, year, close, edit }: { job: MemberJob; records: IncomeRecord[]; year: string; close: () => void; edit: () => void }) {
  const months = Array.from({ length: 12 }, (_, index) => ({ month: index + 1, amount: jobMonthTotal(job, records, index + 1) }));
  return <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-card)] p-5 shadow-sm">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><p className="text-xs font-bold uppercase text-slate-400">{yearRange(job)}</p><h3 className="mt-1 text-lg font-bold">{job.title}</h3><p className="mt-1 text-sm text-slate-400">{job.company}</p></div>
      <div className="flex gap-2"><button onClick={edit} className="rounded-xl border border-[var(--app-border)] px-3 py-2 text-xs font-bold">Sửa</button><button onClick={close} className="rounded-xl border border-[var(--app-border)] px-3 py-2 text-xs font-bold">Đóng</button></div>
    </div>
    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      <Info label="Thời gian" value={yearRange(job)} />
      <Info label="Trạng thái" value={job.status === "active" ? "Đang làm" : "Đã nghỉ"} />
      <Info label={`Tổng thu nhập ${year}`} value={money(jobYearTotal(job, records))} />
    </div>
    <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">{months.map(item => <div key={item.month} className="rounded-xl border border-[var(--app-border)] p-3"><p className="text-xs text-slate-400">Tháng {item.month}</p><b className="text-sm text-emerald-500">{money(item.amount)}</b></div>)}</div>
    {job.note && <p className="mt-5 rounded-xl border border-[var(--app-border)] p-4 text-sm text-slate-500">{job.note}</p>}
  </div>;
}
...
export function MemberJobsPage({ memberId }: { memberId: string }) {
  ...
  return <div className="max-w-6xl space-y-5">
    <button onClick={() => { window.location.href = "/"; }} className="text-sm font-semibold text-indigo-600">← Hồ sơ thành viên</button>
