...
export function BankCardFormPage({ memberId, cardId, mode }: { memberId: string; cardId?: string; mode: FormMode }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const memberPayload = await readJsonSafe<{ ok?: boolean; data?: Member[] } | Member[]>(membersResponse);
      const accountPayload = await readJsonSafe<{ ok?: boolean; data?: BankAccount[] }>(accountsResponse);
      if (!active) return;
      const nextMembers = (memberPayload as { data?: Member[] })?.data || (Array.isArray(memberPayload) ? memberPayload : []);
