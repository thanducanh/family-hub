...
const incomeFormEnd = content.indexOf('function IncomeManagement() {', incomeFormStart);
...
// Table view in LegacyIncomeSheetManagement and IncomeSheetManagement
content = content.replace(
  /<td className="px-4 py-3">\{record\.name\}<\/td>/g,
...
content = content.replace(
  /<div><b>\{record\.name\}<\/b><p className="text-xs text-slate-400">\{formatDateVN\(record\.incomeDate\)} · \{record\.memberName\} · \{record\.category\}<\/p><\/div>/g,
...
// Xoá `jobs={...}` khỏi `<IncomeRecordForm ... />`
content = content.replace(/ jobs=\{incomeData\?.jobs \|\| \[\]\}/g, '');

fs.writeFileSync('src/components/family-app.tsx', content);
console.log('done refactor safe');
