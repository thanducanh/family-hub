...
c = c.replace('import { JobIncomeManager } from "./job-income-manager";\n', '');
c = c.replace('{viewing ? <JobIncomeManager job={viewing} memberId={memberId} records={records} selectedYear={year} onBack={() => setViewing(null)} onUpdated={() => load()} /> : <>', '');
c = c.replace(/<\/>\n    \}/g, '');
c = c.replace('<button onClick={() => { window.location.href = "/"; }} className="text-sm font-semibold text-indigo-600">← Hồ sơ thành viên</button>', '');

c = c.replace('{error && <p className="text-sm text-rose-500">{error}</p>}', '{error && <p className="text-sm text-rose-500">{error}</p>}\n    {viewing && <JobDetail job={viewing} records={records} year={year} close={() => setViewing(null)} edit={() => { window.location.href = `/members/${memberId}/jobs/${viewing.id}/edit`; }} />}');
...
console.log('Fixed member-job-page.tsx');
