...
const startStr = `<div className="mb-3 rounded-[20px] border border-[#E8DCD5] bg-[var(--mobile-card)] p-4 text-[#171018] shadow-[0_6px_18px_rgba(128,0,32,0.07)]">`;
const endStr = `</details>`;
let idxStart = content.lastIndexOf(startStr);
let idxEnd = content.indexOf(endStr, idxStart) + endStr.length;

if (idxStart !== -1 && idxEnd > idxStart) {
  const original = content.substring(idxStart, idxEnd);
...
${original}
  ...
  console.log("Successfully replaced Mobile UI block.");
} else {
